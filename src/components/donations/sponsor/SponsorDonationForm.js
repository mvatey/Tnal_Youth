...
export default function SponsorDonationForm({ initialData = null }) {
  ...
  const sponsorTypeLabel = (type) => {
    if (type === sponsorTypes[0]) return t("donationPage.individual");
    if (type === sponsorTypes[1]) return t("donationPage.institution");
    return t("donationPage.member");
  };
