...
export default function TableRow({ row, rowNumber, onDelete }) {
  return (
    <tr className="h-11 border-b border-border text-center text-sm text-text-secondary last:border-b-0">
      <td className="px-4 font-Regular text-text-secondary">{rowNumber}</td>
      <td className="px-4 font-Regular text-text-secondary">{row.month}</td>
      ...
      <td className="px-4">
        <div className="flex items-center justify-center gap-2">
          <button className="inline-flex h-5 items-center gap-1 rounded-full bg-secondary px-2.5 text-[10px] font-semibold text-white transition hover:bg-secondary-hover">
