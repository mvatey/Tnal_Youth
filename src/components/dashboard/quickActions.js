        ...
        {actions.map((action) => {
          ...
          return (
            <Link
              key={action.id}
              href={action.href}
              className={`flex min-w-0 items-center gap-2 rounded-lg border border-border px-3 py-2.5 text-xs font-medium transition-all duration-200 hover:-translate-y-0.5 hover:border-primary/30 hover:bg-primary-light hover:shadow-sm ${action.bg} ${action.color}`}
            >
