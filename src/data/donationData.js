export const donationStats = [
  {
    id: "monthly",
    label: "ការបរិច្ចាគប្រចាំខែ",
    value: "$2,000",
    growth: "+15%",
    note: "ក្នុងខែនេះ",
    variant: "blue",
  },
  {
    id: "donors",
    label: "អ្នកបរិច្ចាគសរុប",
    value: "200 នាក់",
    growth: "+10%",
    note: "ក្នុងខែនេះ",
    variant: "amber",
  },
];

export const donationRows = [
  { id: 1, month: "មករា", year: "២០២៦", branch: "កណ្ដាល", monthlyRiel: "៛ 340,000", monthlyUsd: "$470", total: "$545" },
  { id: 2, month: "មីនា", year: "២០២៦", branch: "តាកែវ", monthlyRiel: "៛ 500,000", monthlyUsd: "$500", total: "$625" },
  { id: 3, month: "មីនា", year: "២០២៦", branch: "បាត់ដំបង", monthlyRiel: "៛ 200,000", monthlyUsd: "$400", total: "$450" },
  { id: 4, month: "មេសា", year: "២០២៦", branch: "កំពង់ចាម", monthlyRiel: "៛ 300,000", monthlyUsd: "$200", total: "$230" },
  { id: 5, month: "ឧសភា", year: "២០២៦", branch: "កំពង់ធំ", monthlyRiel: "៛ 500,000", monthlyUsd: "$200", total: "$325" },
  { id: 6, month: "មិថុនា", year: "២០២៦", branch: "សៀមរាប", monthlyRiel: "៛ 400,000", monthlyUsd: "$100", total: "$200" },
  { id: 7, month: "កក្កដា", year: "២០២៥", branch: "បន្ទាយមានជ័យ", monthlyRiel: "៛ 500,000", monthlyUsd: "$200", total: "$325" },
  { id: 8, month: "សីហា", year: "២០២៥", branch: "ក្រចេះ", monthlyRiel: "៛ 400,000", monthlyUsd: "$500", total: "$600" },
  { id: 9, month: "កញ្ញា", year: "២០២៥", branch: "ស្ទឹងត្រែង", monthlyRiel: "៛ 600,000", monthlyUsd: "$400", total: "$550" },
  { id: 10, month: "តុលា", year: "២០២៥", branch: "ភ្នំពេញ", monthlyRiel: "៛ 500,000", monthlyUsd: "$300", total: "$425" },
  { id: 11, month: "វិច្ឆិកា", year: "២០២៥", branch: "កំពង់ស្ពឺ", monthlyRiel: "៛ 200,000", monthlyUsd: "$100", total: "$150" },
  { id: 12, month: "ធ្នូ", year: "២០២៥", branch: "ស្ទឹងត្រែង", monthlyRiel: "៛ 400,000", monthlyUsd: "$200", total: "$300" },
  { id: 13, month: "មករា", year: "២០២៥", branch: "ព្រះសីហនុ", monthlyRiel: "៛ 300,000", monthlyUsd: "$250", total: "$325" },
  { id: 14, month: "កុម្ភៈ", year: "២០២៥", branch: "ព្រៃវែង", monthlyRiel: "៛ 600,000", monthlyUsd: "$350", total: "$500" },
  { id: 15, month: "មីនា", year: "២០២៥", branch: "កំពត", monthlyRiel: "៛ 280,000", monthlyUsd: "$180", total: "$250" },
  { id: 16, month: "មេសា", year: "២០២៥", branch: "ស្វាយរៀង", monthlyRiel: "៛ 440,000", monthlyUsd: "$290", total: "$400" },
];

export const addDonationRows = [
  {
    id: 1,
    branch: "ក្រចេះ",
    month: "មករា",
    year: "២០២៦",
    name: "ឌី រីយ៉ា",
    avatar: "/riya.jpg",
    gender: "ស្រី",
    dob: "០៧ កុម្ភៈ ២០០៦",
    realAmount: "",
    dollarAmount: "",
    paymentMethod: "Cash",
  },
  {
    id: 2,
    branch: "តាកែវ",
    month: "មីនា",
    year: "២០២៦",
    name: "តុំ ស៊ីវនាន",
    avatar: "/sivnean.jpg",
    gender: "ស្រី",
    dob: "០៧ កុម្ភៈ ២០០៥",
    realAmount: "",
    dollarAmount: "",
    paymentMethod: "Cash",
  },
  {
    id: 3,
    branch: "ក្រចេះ",
    month: "មករា",
    year: "២០២៦",
    name: "ជា ជិង​ជិង",
    avatar: "/ching.jpg",
    gender: "ស្រី",
    dob: "១២ មិថុនា ២០០៦",
    realAmount: "",
    dollarAmount: "",
    paymentMethod: "ABA",
  },
  {
    id: 4,
    branch: "ក្រចេះ",
    month: "មករា",
    year: "២០២៦",
    name: "យីម ស្រីពៅ",
    avatar: "/sreypov.jpg",
    gender: "ស្រី",
    dob: "១៥ មករា ២០០៤",
    realAmount: "",
    dollarAmount: "",
    paymentMethod: "Cash",
  },
  {
    id: 5,
    branch: "ក្រចេះ",
    month: "មករា",
    year: "២០២៦",
    name: "សេង​ គឹមសួរ",
    avatar: "/kimsour.jpg",
    gender: "ប្រុស",
    dob: "២១ មីនា ២០០៥",
    realAmount: "",
    dollarAmount: "",
    paymentMethod: "Cash",
  },
  {
    id: 6,
    branch: "ក្រចេះ",
    month: "មករា",
    year: "២០២៦",
    name: "ទេព មករា",
    avatar: "/makara.jpg",
    gender: "ប្រុស",
    dob: "០៣ មេសា ២០០៣",
    realAmount: "",
    dollarAmount: "",
    paymentMethod: "Cash",
  },
  {
    id: 7,
    branch: "ក្រចេះ",
    month: "មករា",
    year: "២០២៦",
    name: "សាធ ប៊ុនថន",
    avatar: "/thorn.jpg",
    gender: "ប្រុស",
    dob: "១១ ឧសភា ២០០6",
    realAmount: "",
    dollarAmount: "",
    paymentMethod: "Cash",
  },
  {
    id: 8,
    branch: "ក្រចេះ",
    month: "មករា",
    year: "២០២៦",
    name: "ឈួម​ ម៉ាលីស",
    avatar: "/malis.jpg",
    gender: "ស្រី",
    dob: "២៥ មិថុនា ២០០៥",
    realAmount: "",
    dollarAmount: "",
    paymentMethod: "Cash",
  },
  {
    id: 9,
    branch: "ក្រចេះ",
    month: "មករា",
    year: "២០២៦",
    name: "ឆាយ ចន្ថា",
    avatar: "/chantha.jpg",
    gender: "ស្រី",
    dob: "០៩ កក្កដា ២០០៦",
    realAmount: "",
    dollarAmount: "",
    paymentMethod: "Cash",
  },
  {
    id: 10,
    branch: "ក្រចេះ",
    month: "មករា",
    year: "២០២៦",
    name: "សុធា លីសា",
    avatar: "/lisa.jpg",
    gender: "ស្រី",
    dob: "១៧ សីហា ២០០៦",
    realAmount: "",
    dollarAmount: "",
    paymentMethod: "Cash",
  },
  {
    id: 11,
    branch: "ក្រចេះ",
    month: "មករា",
    year: "២០២៦",
    name: "ទូច សុគន្ធារ៉ា",
    avatar: "/mazer.jpg",
    gender: "ស្រី",
    dob: "២៨ កញ្ញា ២០០៥",
    realAmount: "",
    dollarAmount: "",
    paymentMethod: "Cash",
  },
  {
    id: 12,
    branch: "ក្រចេះ",
    month: "មករា",
    year: "២០២៦",
    name: "អេន ច័ន្ទហេង",
    avatar: "/chanheng.jpg",
    gender: "ស្រី",
    dob: "០៥ តុលា ២០០៦",
    realAmount: "",
    dollarAmount: "",
    paymentMethod: "Cash",
  },
  {
    id: 13,
    branch: "ក្រចេះ",
    month: "មករា",
    year: "២០២៦",
    name: "សុខ លីណា",
    avatar: "/riya.jpg",
    gender: "ស្រី",
    dob: "១២ មិថុនា ២០០៥",
    realAmount: "",
    dollarAmount: "",
    paymentMethod: "Cash",
  },
  {
    id: 14,
    branch: "ក្រចេះ",
    month: "មករា",
    year: "២០២៦",
    name: "សាន វិសាល",
    avatar: "/sivnean.jpg",
    gender: "ប្រុស",
    dob: "១៥ មករា ២០០៤",
    realAmount: "",
    dollarAmount: "",
    paymentMethod: "ABA",
  },
  {
    id: 15,
    branch: "ក្រចេះ",
    month: "មករា",
    year: "២០២៦",
    name: "ជា ស្រីនិច",
    avatar: "/ching.jpg",
    gender: "ស្រី",
    dob: "២១ មីនា ២០០៥",
    realAmount: "",
    dollarAmount: "",
    paymentMethod: "Cash",
  },
  {
    id: 16,
    branch: "ក្រចេះ",
    month: "មករា",
    year: "២០២៦",
    name: "ហេង សុភ័ក្រ",
    avatar: "/sreypov.jpg",
    gender: "ប្រុស",
    dob: "១១ ឧសភា ២០០៤",
    realAmount: "",
    dollarAmount: "",
    paymentMethod: "Wing",
  },
  {
    id: 17,
    branch: "ក្រចេះ",
    month: "មករា",
    year: "២០២៦",
    name: "យ៉ាន សុវណ្ណ",
    avatar: "/kimsour.jpg",
    gender: "ស្រី",
    dob: "០៩ កក្កដា ២០០៤",
    realAmount: "",
    dollarAmount: "",
    paymentMethod: "Cash",
  },
  {
    id: 18,
    branch: "ក្រចេះ",
    month: "មករា",
    year: "២០២៦",
    name: "នី ពិសិដ្ឋ",
    avatar: "/makara.jpg",
    gender: "ប្រុស",
    dob: "១៧ សីហា ២០០៣",
    realAmount: "",
    dollarAmount: "",
    paymentMethod: "Bank Transfer",
  },
  {
    id: 19,
    branch: "ក្រចេះ",
    month: "មករា",
    year: "២០២៦",
    name: "អែម សុជាតា",
    avatar: "/thorn.jpg",
    gender: "ស្រី",
    dob: "២៨ កញ្ញា ២០០៥",
    realAmount: "",
    dollarAmount: "",
    paymentMethod: "Cash",
  },
  {
    id: 20,
    branch: "ក្រចេះ",
    month: "មករា",
    year: "២០២៦",
    name: "ពេជ្រ រតនា",
    avatar: "/malis.jpg",
    gender: "ប្រុស",
    dob: "០៥ តុលា ២០០៤",
    realAmount: "",
    dollarAmount: "",
    paymentMethod: "ABA",
  },
  {
    id: 21,
    branch: "តាកែវ",
    month: "មីនា",
    year: "២០២៦",
    name: "ខៀវ មុនីរ័ត្ន",
    avatar: "/chantha.jpg",
    gender: "ប្រុស",
    dob: "១៨ មេសា ២០០៥",
    realAmount: "",
    dollarAmount: "",
    paymentMethod: "Cash",
  },
  {
    id: 22,
    branch: "តាកែវ",
    month: "មីនា",
    year: "២០២៦",
    name: "ឡុង ស្រីលាភ",
    avatar: "/lisa.jpg",
    gender: "ស្រី",
    dob: "២២ វិច្ឆិកា ២០០៤",
    realAmount: "",
    dollarAmount: "",
    paymentMethod: "Wing",
  },
  {
    id: 23,
    branch: "តាកែវ",
    month: "មីនា",
    year: "២០២៦",
    name: "ម៉ៅ ចាន់រ៉ា",
    avatar: "/mazer.jpg",
    gender: "ប្រុស",
    dob: "០៣ ធ្នូ ២០០៣",
    realAmount: "",
    dollarAmount: "",
    paymentMethod: "Cash",
  },
  {
    id: 24,
    branch: "តាកែវ",
    month: "មីនា",
    year: "២០២៦",
    name: "លី សុភា",
    avatar: "/chanheng.jpg",
    gender: "ស្រី",
    dob: "១៤ កុម្ភៈ ២០០៥",
    realAmount: "",
    dollarAmount: "",
    paymentMethod: "ABA",
  },
  {
    id: 25,
    branch: "កណ្ដាល",
    month: "មករា",
    year: "២០២៦",
    name: "វ៉ា សុវណ្ណារ៉ា",
    avatar: "/rithy.jpg",
    gender: "ប្រុស",
    dob: "០៨ មករា ២០០៤",
    realAmount: "",
    dollarAmount: "",
    paymentMethod: "Cash",
  },
  {
    id: 26,
    branch: "កណ្ដាល",
    month: "មករា",
    year: "២០២៦",
    name: "គង់ ស្រីពេជ្រ",
    avatar: "/theth.jpg",
    gender: "ស្រី",
    dob: "២០ មិថុនា ២០០៥",
    realAmount: "",
    dollarAmount: "",
    paymentMethod: "Bank Transfer",
  },
  {
    id: 27,
    branch: "កណ្ដាល",
    month: "មករា",
    year: "២០២៦",
    name: "សែម វណ្ណៈ",
    avatar: "/vlood.jpg",
    gender: "ប្រុស",
    dob: "១៩ កញ្ញា ២០០៣",
    realAmount: "",
    dollarAmount: "",
    paymentMethod: "Cash",
  },
  {
    id: 28,
    branch: "ភ្នំពេញ",
    month: "កុម្ភៈ",
    year: "២០២៦",
    name: "ចាន់ សុវី",
    avatar: "/vorleak.jpg",
    gender: "ស្រី",
    dob: "២៧ ឧសភា ២០០៤",
    realAmount: "",
    dollarAmount: "",
    paymentMethod: "ABA",
  },
  {
    id: 29,
    branch: "ភ្នំពេញ",
    month: "កុម្ភៈ",
    year: "២០២៦",
    name: "រ៉េត សំណាង",
    avatar: "/branch_leader.jpg",
    gender: "ប្រុស",
    dob: "០៦ សីហា ២០០៥",
    realAmount: "",
    dollarAmount: "",
    paymentMethod: "Cash",
  },
  {
    id: 30,
    branch: "ភ្នំពេញ",
    month: "កុម្ភៈ",
    year: "២០២៦",
    name: "មាស លក្ខិណា",
    avatar: "/member.jpg",
    gender: "ស្រី",
    dob: "១៣ មីនា ២០០៤",
    realAmount: "",
    dollarAmount: "",
    paymentMethod: "Wing",
  },
  {
    id: 31,
    branch: "សៀមរាប",
    month: "មីនា",
    year: "២០២៦",
    name: "ឆេង សុធា",
    avatar: "/secretary.jpg",
    gender: "ប្រុស",
    dob: "២៤ តុលា ២០០៣",
    realAmount: "",
    dollarAmount: "",
    paymentMethod: "Cash",
  },
  {
    id: 32,
    branch: "សៀមរាប",
    month: "មីនា",
    year: "២០២៦",
    name: "ស្រី ច័ន្ទមុនី",
    avatar: "/admin.jpg",
    gender: "ស្រី",
    dob: "១៦ ធ្នូ ២០០៥",
    realAmount: "",
    dollarAmount: "",
    paymentMethod: "ABA",
  },
  {
    id: 33,
    branch: "បាត់ដំបង",
    month: "មេសា",
    year: "២០២៦",
    name: "នួន វិចិត្រ",
    avatar: "/riya.jpg",
    gender: "ប្រុស",
    dob: "០២ កក្កដា ២០០៤",
    realAmount: "",
    dollarAmount: "",
    paymentMethod: "Bank Transfer",
  },
  {
    id: 34,
    branch: "បាត់ដំបង",
    month: "មេសា",
    year: "២០២៦",
    name: "ធី សុម៉ាលី",
    avatar: "/sivnean.jpg",
    gender: "ស្រី",
    dob: "៣០ វិច្ឆិកា ២០០៥",
    realAmount: "",
    dollarAmount: "",
    paymentMethod: "Cash",
  },
];
